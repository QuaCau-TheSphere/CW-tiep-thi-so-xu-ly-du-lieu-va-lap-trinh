...
[[./Authorization sinh ra access token để client sử dụng|Authorization sinh ra access token để client sử dụng]] 
[[./Khi access token hết hạn truy cập, client gửi refresh token đến authorization server để được cấp access token mới|Khi access token hết hạn truy cập, client gửi refresh token đến authorization server để được cấp access token mới]] 